...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _loadData() async {
    try {
      final workouts = await _firestoreService.getWorkoutLogs().timeout(const Duration(seconds: 3));
      final meals = await _firestoreService.getMealLogs().timeout(const Duration(seconds: 3));
      final weights = await _firestoreService.getWeightLogs().timeout(const Duration(seconds: 3));
      if (mounted) {
        setState(() {
  ...
  void _navigateTo(BuildContext context, int index) {
    ...
    if (shellState != null) {
      // Accessing the state through a dynamic cast or a shared controller is better, 
      // but for now, we'll use a direct event or just instructions to the user.
      // Actually, let's use a simpler way: just tell the shell to change its index.
